...
#define BLINK_TIMER (CONFIG_BLINK_PERIOD / 2)
...
void button_press_handler(void *arg)
{
    ...
    if (cur_level == 0)
    {
        ...
        else
        {
            ...
            // if (pressed_time > LONG_PRESS) // longer TRIGER_DURATION
            // {
            //     // long pressed
            ...
            int cur_pressed_length = (pressed_time / LONG_PRESS) % 5;
            if (cur_pressed_length != pressed_length)
            {
                if (cur_pressed_length == 0)
                {
                ...
                {
                    ESP_LOGI(TAG, "Changed blink period.");
                    xTimerChangePeriod(blink_timer, BLINK_TIMER * cur_pressed_length / portTICK_PERIOD_MS, 10);
                    start_blink();
                }
                pressed_length = cur_pressed_length;
            }
        }
    }
    else
