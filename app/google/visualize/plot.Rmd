...
```{r}
esp32 %>% 
  ggplot(aes(x=DATE_TIME, y=BATTERY_VOLT)) +
  geom_point() + 
  #geom_line() +
...
esp32_minimax %>% 
  ...
  ggplot(aes(x=DATE_TIME, y=TEMP, color=SENSOR)) +
  geom_point() + 
  geom_line() +
...
errors %>% 
  ggplot(aes(x=DATE_TIME, y=DELTA)) +
  geom_point()
...
```{r}
esp32 %>% 
  ggplot(aes(x=DATE_TIME, y=SHTC30_HUMI)) +
  geom_point() + 
  geom_line() +
  #scale_colour_viridis_d() + 
  #scale_color_viridis(discrete=TRUE) + 
...
```{r}
esp32 %>% 
  ggplot(aes(x=DATE_TIME, y=BOOT_COUNT)) +
  geom_point() + 
  #scale_colour_viridis_d() + 
  #scale_color_viridis(discrete=TRUE) + 
  theme_bw()
